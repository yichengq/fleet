...
func (r *Registry) GetAllPayloads() []job.JobPayload {
	...
	for _, node := range resp.Node.Nodes {
		ok, err := r.VerifySigning(node.Key, node.Value)
		if err != nil {
			log.Error(err.Error())
			continue
		}
		if !ok {
			log.Error("Fail to verify the job signing")
			continue
		}
...
func (r *Registry) GetPayload(payloadName string) *job.JobPayload {
	...
	ok, err := r.VerifySigning(key, resp.Node.Value)
	if err != nil {
		log.Error(err.Error())
		return nil
	}
	if !ok {
		log.Error("Fail to verify the job")
		return nil
	}
